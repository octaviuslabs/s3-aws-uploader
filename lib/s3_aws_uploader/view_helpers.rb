module S3AwsUploader
  module ViewHelpers
    ...
    def bootstrap_s3_upload(input_field, s3_upload_controller_path)
      ...
      output += %[<button id="#{input_field}_choose_file" type="button" class="#{S3AwsUploader.config.button_style}">choose file</button><div id="#{input_field}_filename" class="file-upload-filename">No file selected.</div>]
      output += %[</div>]
